      ...
      <div className="relative rounded-2xl overflow-hidden border border-[#2a2a2a] shadow-[0_32px_80px_rgba(0,0,0,0.8)]">
        ...
        <div className="flex bg-[#0e0e0e]">
          ...
          <div className="w-14 bg-[#0a0a0a] border-r border-[#1a1a1a] flex flex-col items-center pt-4 pb-6 gap-4">
            <div className="w-8 h-8 bg-orange-500 rounded-lg flex items-center justify-center text-white text-xs font-bold font-display">S</div>
            ...
            <div className="mt-auto w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-red-600 text-[10px] text-white flex items-center justify-center font-bold">JD</div>
          </div>
          ...
          <div className="flex-1 p-4 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-white text-sm font-display font-semibold">Hello, Trader 👋</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: 'Active Signals', value: '342', change: '+12.5%', up: true },
                { label: 'Avg Win Rate', value: '87.4%', change: '+3.2%', up: true },
                { label: 'My Balance', value: '$1,240', change: '-2.1%', up: false },
              ].map((card, i) => (
                <div key={i} className="bg-[#141414] border border-[#1f1f1f] rounded-xl p-3">
                  <p className="text-[10px] text-zinc-500 mb-1 font-mono">{card.label}</p>
                  <p className="text-white font-display font-bold text-base">{card.value}</p>
                  <p className={`text-[10px] font-mono mt-0.5 ${card.up ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <div className="grid grid-cols-5 gap-3">
              ...
              <div className="col-span-3 bg-[#141414] border border-[#1f1f1f] rounded-xl p-3">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[10px] text-zinc-400 font-mono">SIGNAL PERFORMANCE</span>
                  <span className="text-[10px] text-orange-400 font-mono">+18.3% ↑</span>
                </div>
                <div className="flex items-end gap-1 h-16">
                  {[40, 55, 35, 70, 45, 85, 60, 90, 65, 88, 72, 95].map((h, i) => (
                    <div key={i} className="flex-1 rounded-sm"
                      style={{
                        height: `${h}%`,
                        background: i >= 9 ? '#f97316' : i >= 6 ? 'rgba(249,115,22,0.4)' : '#1f1f1f'
                      }} />
                  ))}
                </div>
              ...
              <div className="col-span-2 bg-[#141414] border border-[#1f1f1f] rounded-xl p-3">
                <span className="text-[10px] text-zinc-400 font-mono block mb-2">LIVE SIGNALS</span>
                <div className="flex flex-col gap-2">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between">
                      <span className="text-[10px] text-white font-mono">{s.pair}</span>
                      <span className={`text-[9px] font-bold font-mono px-1.5 py-0.5 rounded bg-[#1f1f1f] ${s.color}`}>
                        {s.dir}
                      </span>
                    </div>
                  ))}
...
export default function LandingPage() {
  ...
  const [animatedNumbers, setAnimatedNumbers] = useState({ signals: 0, winRate: 0, providers: 0, price: 0 });
  const [isVisible, setIsVisible] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [liveFeed, setLiveFeed] = useState<{ id: number; user: string; action: 'bought' | 'published'; signal: string; amount?: string; price?: string; time: string; }[]>([
    { id: 1, user: 'Trader_Jason', action: 'bought', signal: 'BTC/USD', amount: '4.99', time: 'Just now' },
    { id: 2, user: 'CryptoGirl', action: 'bought', signal: 'ETH/USD', amount: '3.99', time: '12s ago' },
    { id: 3, user: 'ForexKing', action: 'published', signal: 'EUR/USD', price: '2.99', time: '45s ago' },
    { id: 4, user: 'StockMaster', action: 'bought', signal: 'NVDA', amount: '4.99', time: '1m ago' },
  ]);

  useEffect(() => {
    const interval = setInterval(() => {
      setLiveFeed(prev => {
        const action = Math.random() > 0.5 ? 'bought' as const : 'published' as const;
        const item = {
          id: Date.now(),
    ...
    <div className="min-h-screen bg-[#080808] overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <Link href="/" className="flex items-center gap-2.5 group">
              <div className="w-9 h-9 bg-orange-500 rounded-lg flex items-center justify-center font-display font-bold text-white text-sm shadow-lg shadow-orange-500/30 group-hover:shadow-orange-500/50 transition-shadow">
                S
              </div>
            ...
            <div className="hidden md:flex items-center gap-1">
              {[
                { href: '/marketplace', label: 'Browse Signals' },
              ...
              {user ? (
                <>
                  <Link href={getDashboardLink()} className="px-3 py-2 text-sm text-zinc-400 hover:text-white rounded-lg hover:bg-white/5 transition-all">
                    {getDashboardText()}
                  </Link>
                  <Link href="/marketplace" className="ml-1 px-4 py-2 bg-orange-500 hover:bg-orange-400 text-white text-sm font-semibold rounded-lg transition-all shadow-lg shadow-orange-500/25 hover:shadow-orange-500/40">
                    Start Trading →
                  </Link>
                </>
              ) : (
                <>
                  <Link href="/signin" className="px-3 py-2 text-sm text-zinc-400 hover:text-white rounded-lg hover:bg-white/5 transition-all">
                    Sign In
                  </Link>
                  <Link href="/signup" className="ml-1 px-4 py-2 bg-orange-500 hover:bg-orange-400 text-white text-sm font-semibold rounded-lg transition-all shadow-lg shadow-orange-500/25 hover:shadow-orange-500/40">
                    Sign Up Free →
                  </Link>
                </>
              )}
          ...
          {/* Mobile menu */}
          {mobileMenuOpen && (
            <div className="md:hidden py-4 border-t border-[#1f1f1f] flex flex-col gap-1">
              {[
                { href: '/marketplace', label: 'Browse Signals' },
                { href: '/provider-signup', label: 'Sell Signals' },
                { href: '/get-verified', label: 'Get Verified' },
                ...
                <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}
                  className="px-3 py-2.5 text-sm text-zinc-400 hover:text-white rounded-lg hover:bg-white/5 transition">
                  {item.label}
                </Link>
              ))}
              ...
              {user ? (
                <>
                  ...
                  <Link href="/marketplace" onClick={() => setMobileMenuOpen(false)}
                    className="mx-3 mt-1 py-2.5 text-center bg-orange-500 text-white text-sm font-semibold rounded-lg">
                    Start Trading →
                  </Link>
                </>
              ) : (
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8 text-center">
        ...
        >
          ...
          <span className="text-xs text-orange-300 font-mono tracking-wide">🔥 LIVE SIGNALS AVAILABLE NOW</span>
        </motion.div>
        ...
        >
          Join 10,000+ traders who get profitable signals daily. No expensive mentorships.
          Pay per signal, keep 80% of your profits.
        </motion.p>
      ...
      {/* ── Stats Bar ─────────────────────────────────────────── */}
      <section id="stats-section" className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { value: `${animatedNumbers.signals}+`, label: 'Active Signals', suffix: '' },
            { value: `${animatedNumbers.winRate}%`, label: 'Avg Win Rate', suffix: '' },
            { value: `${animatedNumbers.providers}+`, label: 'Expert Providers', suffix: '' },
            { value: `$${(animatedNumbers.price / 100).toFixed(2)}`, label: 'Starting Price', suffix: '' },
          ].map((stat, i) => (
            <div key={i} className="bg-[#111] border border-[#1f1f1f] rounded-2xl p-5 text-center hover:border-orange-500/30 transition-colors group">
              <div className="font-display font-bold text-3xl text-orange-500 group-hover:text-orange-400 transition-colors font-mono">
                {stat.value}
              </div>
              <div className="text-zinc-500 text-sm mt-1 font-mono tracking-wide">{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <div className="bg-[#111]/80 border border-[#1f1f1f] rounded-xl px-4 py-3 overflow-hidden">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[11px] text-zinc-500 font-mono tracking-widest">📡 LIVE ACTIVITY FEED</span>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar flex-wrap">
            {liveFeed.map(item => (
              <div key={item.id} className="text-[11px] bg-[#0d0d0d] border border-[#1f1f1f] rounded-full px-3 py-1 whitespace-nowrap font-mono">
                <span className="text-orange-400">{item.user}</span>
                <span className="text-zinc-600 mx-1">{item.action === 'bought' ? 'bought' : 'published'}</span>
      ...
      >
        <motion.div variants={fadeUp} className="text-center mb-12">
          <span className="text-xs text-orange-500 font-mono tracking-widest uppercase">Simple Process</span>
          <h2 className="font-display font-bold text-3xl md:text-4xl text-white mt-2">How SignalHub Works</h2>
        </motion.div>
        <div className="grid md:grid-cols-3 gap-5">
          {[
            { icon: '🛒', step: '01', title: 'Buy Signals', desc: 'Choose from 500+ signals. Pay as low as $2.99. Full analysis revealed instantly.' },
            { icon: '📡', step: '02', title: 'Sell Signals', desc: 'Become a provider, share your analysis, and earn 80% of every sale you make.' },
            { icon: '💸', step: '03', title: 'Get Paid', desc: 'Withdraw earnings weekly via bank transfer, crypto, or PayPal. Zero hidden fees.' },
          ].map((item, i) => (
      ...
      >
        <div className="text-center mb-10">
          <span className="text-xs text-orange-500 font-mono tracking-widest uppercase">Live Data</span>
          <h2 className="font-display font-bold text-3xl md:text-4xl text-white mt-2 mb-2">Top Performing Signals</h2>
          <p className="text-zinc-500 text-sm">Real-time signals from our highest-rated providers</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1f1f1f]">
                  {['Pair', 'Price', 'Provider', 'Win Rate', 'Signal Price', 'Activity', ''].map((h, i) => (
                    <th key={i} className="text-left px-5 py-4 text-zinc-600 text-xs font-mono tracking-widest uppercase">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {(topSignals as Record<string, typeof topSignals.forex>)[activeTab].map((signal, idx) => (
                  <tr key={idx} className="border-b border-[#161616] hover:bg-white/[0.02] transition-colors group">
                    <td className="px-5 py-4">
                      <span className="text-white font-display font-semibold">{signal.pair}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="text-white font-mono text-sm">{signal.price}</span>
                      <span className={`ml-2 text-xs font-mono ${signal.change.startsWith('+') ? 'text-green-400' : 'text-red-400'}`}>
                        {signal.change}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="text-zinc-300 text-sm">{signal.provider}</span>
                      <div className="text-xs text-yellow-500 font-mono">★ {signal.winRate} win</div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-16 bg-[#1f1f1f] rounded-full h-1.5">
                          <div className="bg-green-500 rounded-full h-1.5 transition-all" style={{ width: signal.winRate }} />
                        </div>
                        <span className="text-green-400 text-xs font-mono">{signal.winRate}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span className="text-orange-400 font-display font-bold">{signal.signalPrice}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="text-xs text-zinc-600 font-mono">📊 {signal.buyers} bought today</span>
                    </td>
                    <td className="px-5 py-4">
        ...
        <p className="text-center text-zinc-700 text-xs font-mono mt-4">† 500+ more signals available. Prices vary by provider.</p>
      </motion.section>
      ...
      >
        <motion.div variants={fadeUp} className="text-center mb-10">
          <span className="text-xs text-orange-500 font-mono tracking-widest uppercase">Verified Traders</span>
          <h2 className="font-display font-bold text-3xl md:text-4xl text-white mt-2 mb-2">Top Providers</h2>
          <p className="text-zinc-500 text-sm">Real performance. Full transparency.</p>
        </motion.div>
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-display font-bold text-white">{provider.name}</h3>
                  </div>
                ...
                <div className="text-right">
                  <div className="text-yellow-500 text-sm font-mono">★ {provider.rating}</div>
                  <div className="text-green-400 text-xs font-mono">{provider.winRate} win</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 pt-4 border-t border-[#1a1a1a]">
                <div>
                  <p className="text-zinc-600 text-xs font-mono mb-0.5">TOTAL SIGNALS</p>
                  <p className="text-white font-display font-semibold">{provider.totalSignals}</p>
                </div>
                <div>
                  <p className="text-zinc-600 text-xs font-mono mb-0.5">EARNINGS</p>
                  <p className="text-green-400 font-display font-semibold">{provider.earnings}</p>
                </div>
              </div>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div variants={fadeUp} className="text-center mb-10">
            <span className="text-xs text-orange-500 font-mono tracking-widest uppercase">Social Proof</span>
            <h2 className="font-display font-bold text-3xl md:text-4xl text-white mt-2 mb-2">
              Trusted by 10,000+ Traders
            </h2>
            <p className="text-zinc-500 text-sm">Real results from real people</p>
          </motion.div>
            ...
            {testimonials.map((t, i) => (
              <motion.div key={i} variants={fadeUp}
                ...
                <div className="flex gap-0.5 mb-3">
                  {'★★★★★'.split('').map((s, j) => (
                    <span key={j} className="text-yellow-500 text-sm">{s}</span>
                  ))}
                </div>
                <p className="text-zinc-400 text-sm leading-relaxed italic">"{t.text}"</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <footer className="relative z-10 border-t border-[#1a1a1a] py-10">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <Link href="/" className="inline-flex items-center gap-2 mb-4 group">
            <div className="w-7 h-7 bg-orange-500 rounded-md flex items-center justify-center font-display font-bold text-white text-xs">S</div>
            <span className="font-display font-bold text-base">
              <span className="text-white">Signal</span><span className="text-orange-500">Hub</span>
            </span>
          </Link>
          <p className="text-zinc-700 text-xs font-mono">© 2025 Signal Hub. All rights reserved.</p>
          <p className="text-zinc-800 text-xs font-mono mt-1">Not financial advice. Trade responsibly.</p>
        </div>
      </footer>

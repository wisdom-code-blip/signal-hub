...
const TOPICS = ['General Inquiry', 'Custom Signal Platform', 'Partnership', 'Bug Report', 'Billing', 'Other'];

const CONTACT_INFO = [
  { icon: '📧', label: 'Email', value: 'hello@signalhub.io', href: 'mailto:hello@signalhub.io' },
    ...
    <div className="min-h-screen bg-[#080808] overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-20">
        ...
        <div className="text-center mb-16">
          <span className="text-xs text-orange-500 font-mono tracking-widest">GET IN TOUCH</span>
          <h1 className="font-display font-bold text-4xl md:text-5xl text-white mt-2 mb-3">Contact Us</h1>
          <p className="text-zinc-400 max-w-md mx-auto text-sm leading-relaxed">
            Questions, partnerships, or want a custom trading platform built? We'd love to hear from you.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-4">
                {CONTACT_INFO.map((c, i) => (
                  <a key={i} href={c.href}
                    ...
                    <div>
                      <p className="text-[10px] text-zinc-600 font-mono">{c.label}</p>
                      <p className="text-zinc-300 text-sm font-mono group-hover:text-orange-400 transition-colors">{c.value}</p>
                    </div>
                  </a>
                ))}
              </div>
            ...
            <div className="bg-gradient-to-br from-orange-500/10 to-red-500/5 border border-orange-500/20 rounded-2xl p-6">
              <h3 className="font-display font-semibold text-white mb-2">🚀 Custom Platform?</h3>
              <p className="text-zinc-400 text-xs font-mono leading-relaxed mb-4">
                I build custom signal marketplaces, trading communities, and fintech dashboards for businesses. Starting at ₦50,000.
              </p>
              <div className="space-y-1.5 text-xs font-mono text-zinc-500">
                {['Custom signal marketplace', 'Trading community platform', 'Fintech dashboard', 'Mobile app integration'].map((item, i) => (
            ...
            <div className="bg-[#111] border border-[#1f1f1f] rounded-2xl p-5">
              <h3 className="font-display font-semibold text-white text-sm mb-3">Response Times</h3>
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between">
                    <span className="text-zinc-600 text-xs font-mono">{r.type}</span>
                    <span className="text-zinc-400 text-xs font-mono">{r.time}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-3">
            {!sent ? (
              <form onSubmit={handleSubmit} className="bg-[#111] border border-[#1f1f1f] rounded-2xl p-7 space-y-5">
                ...
                {/* Topic */}
                <div>
                  <label className="text-[10px] text-zinc-500 font-mono tracking-widest block mb-2">TOPIC</label>
                  <div className="flex flex-wrap gap-2">
                    {TOPICS.map(t => (
                      <button key={t} type="button" onClick={() => update('topic', t)}
                ...
                {/* Message */}
                <div>
                  <label className="text-[10px] text-zinc-500 font-mono tracking-widest block mb-1.5">MESSAGE</label>
                  <textarea
                    value={form.message}
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <Link href="/" className="text-orange-400 hover:text-orange-300 text-sm font-mono transition-colors">
                  ← Back to homepage
                </Link>
              </motion.div>
            )}
